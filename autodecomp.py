...
def main():
    ...
    for single_filtered_asm_yaml_stuff in asm_yaml_stuff:
        try:
            subprocess.run("./build.sh", check=True)
            ...
            pprint(pragmas)
            if not pragmas:
                print(f"No #pragma GLOBAL_ASM found in {c_file}.")
                    ...
                    with tempfile.TemporaryDirectory() as d:
                        ...
                        try:
                            ...
                            for post_decomp_var in post_decomp_variables:
                                if post_decomp_var["name"] not in pre_decomp_variable_names and post_decomp_var[
                                    "name"
                                ] not in [ctx_variable_names]:
            ...
            log.error(f"complete failure for yaml file: {single_filtered_asm_yaml_stuff}.\nException: {exc}")
